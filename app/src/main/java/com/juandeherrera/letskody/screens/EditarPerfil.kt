...
@Composable
fun PantallaEditarPerfil(controladorNavegacion: NavController) {
    ...
    // variables para manejar el número de teléfono
    ...
    when (usuario) {
        null -> {
            ...
            /* aquí ya existe el usuario */

            // se busca en la lista de países aquel cuyo prefijo coincida con el del teléfono del usuario
            val paisEncontrado = paises.firstOrNull { usuario!!.telefonoUsuario.startsWith(prefix = it.prefijo) }

            // si se encuentra un país cuyo prefijo coincida con el del teléfono del usuario
            if (paisEncontrado != null) {
                paisSeleccionado = paisEncontrado  // se asigna el país como país seleccionado en el desplegable

                // se elimina el prefijo del país del teléfono para obtener el número del teléfono
                telefono = usuario!!.telefonoUsuario.removePrefix(paisEncontrado.prefijo).takeLast(n = 9)
            }
            else {
                telefono = usuario!!.telefonoUsuario.takeLast(n = 9) // si no hay ninguna coincidencia se obtiene el número de teléfono
            }
        }
    }
    ...
    var sexoSeleccionado by remember { mutableStateOf(value = usuario!!.sexoUsuario) }   // sexo seleccionado
