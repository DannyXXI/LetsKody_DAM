...
// función auxiliar para cargar la barra superior para pantallas sin menu lateral
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun BarraSuperiorSinMenuLateral(titulo: String, fuenteTipografica: FontFamily, controladorNavegacion: NavController, estadoMenuDesplegable: Boolean, abrirMenuDesplegable: () -> Unit, cerrarMenuDesplegable: () -> Unit, cerrarSesionUsuario: () -> Unit) {
    // barra superior
    CenterAlignedTopAppBar(
        modifier = Modifier.height(90.dp),  // altura de la barra superior
        title = {
        ...
        navigationIcon = {
            // botón de la izquierda (abrir el menu lateral)
            IconButton(
                onClick = { controladorNavegacion.popBackStack() }  // al pulsarlo se vuelve a la pantalla anterior
            ){
                Icon(
